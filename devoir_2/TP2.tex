...
Exprimer ce problème de minimisation}
	...
	\begin{eqs*}	
		\nabla \hat{R}(\theta) = \frac{\delta\hat{R}}{\delta\theta}(\theta^{*}) = 0
	\end{eqs*}
...
\subsection{4. Exprimer le gradient du risque empirique}
	...
	\begin{eqs*}	
		= \frac{d}{d\theta} (\sum_{i=1}^{n} (f_{\theta}(x^{(i)}) - t^{(i)})^{2})
	\end{eqs*}

	\begin{eqs*}
		= \frac{d}{d\theta} (\sum_{i=1}^{n} (\begin{bmatrix}
									\theta_{1} \\
									\theta_{2} \\
									\vdots \\
									\theta_{d}
	...
	\begin{eqs*}
		= \frac{d}{d\theta} (\sum_{i=1}^{n}(w^{T}x^{(i)} + b - t^{(i)})^{2})
	\end{eqs*}
